...
mat4 rotationMatrix(vec3 axis, float angle)
{
    ...
    return mat4(oc * axis.x * axis.x + c,           oc * axis.x * axis.y -
axis.z * s,  oc * axis.z * axis.x + axis.y * s,  0.0, oc * axis.x * axis.y +
axis.z * s,  oc * axis.y * axis.y + c,           oc * axis.y * axis.z - axis.x *
s,  0.0, oc * axis.z * axis.x - axis.y * s,  oc * axis.y * axis.z + axis.x * s,
...
float getHeight(vec3 p) // x,z,time
{
  ...
  return 0.3 -
         0.5 * fnoise(vec3(0.5 * (p.x + 0.0 * iTime), 0.5 * p.z, 0.4 * iTime));
}
