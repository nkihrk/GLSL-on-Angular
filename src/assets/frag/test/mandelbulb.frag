...
vec3 calcNormal(in vec3 pos, in float t, in float px) {
  ...
  return normalize(
      e.xyy * map(pos + e.xyy, tmp) + e.yyx * map(pos + e.yyx, tmp) +
      e.yxy * map(pos + e.yxy, tmp) + e.xxx * map(pos + e.xxx, tmp));
}
...
vec3 render(in vec2 p, in mat4 cam) {
  ...
  if (t < 0.0) {
    ...
    col +=
        5.0 * vec3(0.8, 0.7, 0.5) * pow(clamp(dot(rd, light1), 0.0, 1.0), 32.0);
  }
  // color fractal
  else {
    // color
    ...
    float spe1 =
        pow(clamp(dot(nor, hal), 0.0, 1.0), 32.0) * dif1 *
        (0.04 + 0.96 * pow(clamp(1.0 - dot(hal, light1), 0.0, 1.0), 5.0));
    // bounce
    ...
    col += spe1 * 15.0;
    // col
    // += 8.0*vec3(0.8,0.9,1.0)*(0.2+0.8*occ)*(0.03+0.97*pow(fac,5.0))*smoothstep(0.0,0.1,ref.y
    // )*softshadow( pos+0.01*nor, ref, 2.0 ); col = vec3(occ*occ);
  }
