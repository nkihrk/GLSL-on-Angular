...
z^8 = r^8 * (sin(8*theta)*cos(8*phi) + i cos(8*theta) + j
sin(8*theta)*sin(8*theta)
...
vec3 nor(in vec3 pos) {
  ...
  return normalize(vec3(f(pos + eps.xyy).x - f(pos - eps.xyy).x,
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 rd = normalize(uv.x * cs + uv.y * cu +
                      3.0 * cf); // transform from view to world
  ...
  if (res.x > 0.0) {
    ...
    float spe =
        max(0.0, pow(clamp(dot(sundir, reflect(rd, n)), 0.0, 1.0), 10.0));
  ...
  col *= 0.5 + 0.5 * pow(16.0 * q.x * q.y * (1.0 - q.x) * (1.0 - q.y),
                         0.7); // vigneting
