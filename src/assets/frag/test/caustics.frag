// https://www.shadertoy.com/view/WlXfWr
...
float CyclicTime() { return mod(iTime, 30.); }

float WaveAmplitude() { return MaxWaveAmplitude * exp(-CyclicTime() / 10.); }

float WaterWave(vec3 a) {
  float wave = 0.0;
  vec3 org = a;
  float amp = WaveAmplitude();
  wave += amp * min(pow(distance(org, vec3(0.0)), -1.), 2.0) *
          sin((2. * a.x * a.x + 2. * a.z * a.z) - 15. * CyclicTime());
  return wave;
}
...
float PoolBottom(vec3 a) { return a.y + DepthPool + .01; }

float BackWall(vec3 a) { return a.z + HalfSizePool + .01; }

float LeftWall(vec3 a) { return a.x + HalfSizePool + .01; }
...
float Pool(vec3 a) { return min(PoolBottom(a), min(LeftWall(a), BackWall(a))); }
...
float Scene(vec3 a) { return min(WaterSurface(a), min(Ball(a), Pool(a))); }
...
bool IsWater(vec3 pos) { return (pos.y < (WaterHeight - MaxWaveAmplitude)); }

bool IsPool(vec3 pos) { return Pool(pos) < 0.01; }

bool IsBall(vec3 pos) { return Ball(pos) < 0.01; }
...
vec3 Normal(vec3 a) {
  ...
  float s = Scene(a);
  return normalize(
      vec3(Scene(a + e.xyy) - s, Scene(a + e.yxy) - s, Scene(a + e.yyx) - s));
}
...
vec3 Lighting(vec3 at, vec3 normal, vec3 eye, MaterialInfo m, vec3 lColor,
              vec3 lPos) {
  vec3 lDir = lPos - at;
  ...
  if (m.Shininess > 0.) {
    ...
    color += lColor * pow(max(0., dot(normal, h)), m.Shininess) *
             (m.Shininess + 8.) / 25.;
  }
...
vec3 Shade(vec3 rpos, vec3 rdir, float t) {
  ...
  if (waterSurface) {
    ...
    waterSurfaceLight =
        Lighting(pos, nor, rpos, Material(pos), light, LightSource);
...
vec3 Camera(vec2 px) {
  ...
  vec3 rayDirection = LookAt(rayStart, vec3(0, sin(iTime * 0.5) - 1.5, 0),
                             normalize(vec3(uv, -3.)));
